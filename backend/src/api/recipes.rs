use std::{sync::Arc, default, time::Duration};

use axum::{routing::{post, MethodRouter, get}, Json, http::StatusCode, extract::Path};
...
pub fn update(foodbase: Arc<FoodBase>) -> MethodRouter<> {
    let db = foodbase.clone();
    post(|Path(recipe_id) :Path<i32>, recipe: Json<Recipe>| async move {
        let recipe = crate::db::Recipe  {
            recipe_id: recipe_id,
            name: recipe.name.to_owned(),
            comment: recipe.comment.to_owned()
...
pub fn update_entry(foodbase: Arc<FoodBase>) -> MethodRouter<> {
    let db = foodbase.clone();
    post(|Path(recipe_id): Path<i32>, Json(entries):  Json<Vec<RecipeIngrdient>>| async move {
        let result = db.update_recipe_entries(recipe_id, entries.into_iter()).await;
        match result {
            Ok(_) => StatusCode::CREATED,
            _ => StatusCode::INTERNAL_SERVER_ERROR
        }
...
pub fn update_steps(foodbase: Arc<FoodBase>) -> MethodRouter<> {
    let db = foodbase.clone();
    post(|Path(recipe_id): Path<i32>, Json(steps):  Json<Vec<RecipeStepSerializable>>| async move {
