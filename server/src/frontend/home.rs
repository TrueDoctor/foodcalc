use axum::response::IntoResponse;
use foodlib_new::user::User;
use maud::{html, Markup};

use crate::{
    frontend::{ingredients_tab::render_ingredients_view, LOGIN_URL},
    FoodLib,
};

pub(crate) fn home_router() -> axum::Router<crate::MyAppState> {
    axum::Router::new().route("/", axum::routing::get(home_view))
}

#[axum::debug_handler]
pub async fn home_view(user: Option<User>, foodlib: FoodLib) -> impl IntoResponse {
    (
        [("HX-Replace-Url", "ingredients")],
        content(foodlib, user).await,
    )
}

pub async fn content(foodlib: FoodLib, user: Option<User>) -> Markup {
    html! {
        div class="flex flex-col items-center justify-center mb-16" {
            div id="content" class="w-3/4 flex flex-col items-center justify-center" {
                (render_ingredients_view(foodlib, user).await)
            }
        }
    }
}

pub fn navbar(user: Option<User>, host: &str) -> Markup {
    html! {
        //Warn local developers if their server is pointed at the production database.
        @let is_localhost = host == "localhost" || host == "127.0.0.1";
        @let on_prod_db = std::env::var("DATABASE_URL").unwrap().ends_with("food_calc");
        @if is_localhost && on_prod_db {
            dialog open="true" class="bg-btn-cancel-normal text-white rounded-lg w-full" id="banner" {
                div class="p-4 flex flex-col items-center justify-center gap-4" {
                    p class="text-2xl" {
                        "WARNING: You are using the production database!"
                    }
                    p class="text-lg" {
                        (std::env::var("DATABASE_URL").unwrap())
                    }
                    button class="btn bg-black w-1/2" hx-on:click="document.getElementById('banner').remove()" {
                        "Close"
                    }
                }
            }
        }

        div class="flex justify-end mx-16 mt-2" {
            (user_menu(user.as_ref()))
        }

        div class="
            flex items-center justify-between flex-wrap
            bg-navbar text-white
            mx-16 my-4
            rounded-xl shadow-xl overflow-hidden
        " {
            (navbutton("Ingredients", "/ingredients"))
            (navbutton("Recipes", "/recipes"))
            @if user.is_some() {
                (navbutton("Events", "/events"))
                (navbutton("Inventories", "/inventories"))
            }
        }
    }
}

fn user_menu(user: Option<&User>) -> Markup {
    html! {
        @match user {
            Some(u) => {
                details class="user-menu relative" {
                    summary class="flex items-center gap-2 cursor-pointer list-none
                        bg-navbar text-white
                        transition ease-in-out duration-200
                        rounded-xl px-4 py-2 shadow
                        hover:bg-blue-800" {
                        svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" class="h-4 w-4 text-white" {
                            path fill="currentColor" d="M224 256A128 128 0 1 0 224 0a128 128 0 1 0 0 256zm-45.7 48C79.8 304 0 383.8 0 482.3 0 498.7 13.3 512 29.7 512l388.6 0c16.4 0 29.7-13.3 29.7-29.7 0-98.5-79.8-178.3-178.3-178.3l-91.4 0z" {}
                        }
                        span { (u.username) }
                        svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" class="h-3 w-3 text-white" {
                            path fill="currentColor" d="M137.4 374.6c12.5 12.5 32.8 12.5 45.3 0l128-128c9.2-9.2 11.9-22.9 6.9-34.9s-16.6-19.8-29.6-19.8L32 192c-12.9 0-24.6 7.8-29.6 19.8s-2.2 25.7 6.9 34.9l128 128z" {}
                        }
                    }
                    div class="absolute right-0 mt-1 min-w-[12rem] bg-navbar text-white rounded-xl shadow-xl overflow-hidden z-50" {
                        a hx-get="/admin/groups" hx-target="#content" hx-push-url="true"
                            class="block px-4 py-3 cursor-pointer hover:bg-blue-800" { "Groups" }
                        @if u.is_admin {
                            a hx-get="/admin/users" hx-target="#content" hx-push-url="true"
                                class="block px-4 py-3 cursor-pointer hover:bg-blue-800" { "Users" }
                        }
                        a hx-get="/auth/logout" hx-target="#content"
                            class="block px-4 py-3 cursor-pointer hover:bg-blue-800 border-t border-blue-900" { "Log out" }
                    }
                }
            }
            None => {
                a hx-get=(LOGIN_URL) hx-target="#content"
                    class="flex items-center gap-2 cursor-pointer
                    bg-navbar text-white
                    transition ease-in-out duration-200
                    rounded-xl px-4 py-2 shadow
                    hover:bg-blue-800" {
                    span { "Log in" }
                    svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="h-4 w-4 text-white" {
                        path fill="currentColor" d="M217.9 105.9L340.7 228.7c7.2 7.2 11.3 17.1 11.3 27.3s-4.1 20.1-11.3 27.3L217.9 406.1c-6.4 6.4-15 9.9-24 9.9c-18.7 0-33.9-15.2-33.9-33.9l0-62.1L32 320c-17.7 0-32-14.3-32-32l0-64c0-17.7 14.3-32 32-32l128 0 0-62.1c0-18.7 15.2-33.9 33.9-33.9c9 0 17.6 3.6 24 9.9zM352 416l64 0c17.7 0 32-14.3 32-32l0-256c0-17.7-14.3-32-32-32l-64 0c-17.7 0-32-14.3-32-32s14.3-32 32-32l64 0c53 0 96 43 96 96l0 256c0 53-43 96-96 96l-64 0c-17.7 0-32-14.3-32-32s14.3-32 32-32z" {}
                    }
                }
            }
        }
    }
}

fn navbutton(text: &str, link: &str) -> Markup {
    html! {
        a hx-get=(link) hx-target="#content" hx-push-url="true" class="flex flex-col items-center grow
            transition ease-in-out transition duration-200 ease-in-out
            rounded-xl p-6
            hover:shadow-inner hover:bg-blue-800" { (text) }
    }
}
